...
class SolverMinMax:
    ...
    def init_model(self):
        ...
        r = self.model.addVars(
            self.T, lb=0, ub=self.L, vtype=GRB.CONTINUOUS, name="r")
        p = self.model.addVars(self.arcs, name="p", vtype=GRB.INTEGER)
        ...
        c1 = self.model.addConstrs(((quicksum(
            self.c[i, j]*x[i, j, k] for i in self.N for j in self.N if i != j))
            <= z for k in self.K), name="c1")
